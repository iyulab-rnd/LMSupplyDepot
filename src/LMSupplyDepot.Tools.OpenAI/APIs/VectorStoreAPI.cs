...
public class VectorStoreAPI
{
    ...
    /// </summary>
    /// <param name="fileIds">Collection of file IDs to include in the vector store</param>
    /// <param name="name">Optional name for the vector store</param>
    /// <returns>The created vector store information</returns>
    public async Task<VectorStore> CreateVectorStoreAsync(IEnumerable<string> fileIds, string name = null)
    {
        name ??= $"vectorstore-{DateTime.UtcNow:yyyyMMddHHmmss}";
        Console.WriteLine($"Creating vector store: {name}");
        ...
        try
        {
            ...
            // Add each file ID to the collection using collection methods
    ...
    /// </summary>
    /// <param name="vectorStoreId">ID of the vector store to monitor</param>
    /// <param name="maxAttempts">Maximum number of polling attempts</param>
    /// <param name="delaySeconds">Delay in seconds between polling attempts</param>
    /// <returns>The final state of the vector store</returns>
    ...
    public List<VectorStore> ListVectorStores()
    {
        try
        {
            ...
            // 컬렉션 결과를 리스트로 변환
            List<VectorStore> storesList = new List<VectorStore>();
            foreach (var vectorStore in vectorStoresCollection)
            {
                storesList.Add(vectorStore);
            }
    ...
    /// </summary>
    /// <param name="vectorStoreId">ID of the vector store to delete</param>
    ...
    /// </summary>
    /// <param name="vectorStoreId">ID of the vector store</param>
    /// <param name="fileId">ID of the file to add</param>
    /// <returns>Result of the file association operation</returns>
    public async Task<VectorStoreFileAssociation> AddFileToVectorStoreAsync(string vectorStoreId, string fileId)
    {
        ...
        try
        {
            ...
            var fileAssociation = operation.Value;
    ...
    /// <param name="status">Optional status filter (e.g., "completed", "in_progress", etc.)</param>
    /// <returns>Collection of file associations</returns>
    public List<VectorStoreFileAssociation> ListVectorStoreFiles(string vectorStoreId, int? limit = null, string status = null)
    {
        try
        {
            ...
            List<VectorStoreFileAssociation> filesList = new List<VectorStoreFileAssociation>();
    ...
    /// </summary>
    /// <param name="vectorStoreId">ID of the vector store</param>
    /// <param name="fileIds">Collection of file IDs to add</param>
    /// <returns>Batch job operation that can be monitored</returns>
