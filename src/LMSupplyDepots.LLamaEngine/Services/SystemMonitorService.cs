...
public record SystemMetrics
{
    ...
    public Dictionary<string, GpuMetrics> GpuMetrics { get; init; } = new();
    public Dictionary<string, DiskMetrics> DiskMetrics { get; init; } = new();
}
...
public class SystemMonitorService : ISystemMonitorService, IDisposable
{
    ...
    private double GetCpuUsage()
    {
        lock (_lock)
        {
            try
            {
                ...
                if (elapsedTime < 0.1) // 최소 100ms 간격 필요
                {
    ...
    public SystemMetrics GetCurrentMetrics()
    {
        ...
        catch (Exception ex)
        {
            ...
            return new SystemMetrics
            {
                ...
                TotalMemoryBytes = 0,
                GpuMetrics = new Dictionary<string, GpuMetrics>(),
                DiskMetrics = new Dictionary<string, DiskMetrics>()
            };
        }
    }
    ...
    private Dictionary<string, GpuMetrics> GetGpuMetrics()
    {
        ...
        try
        {
            // OpenCL이나 다른 크로스 플랫폼 GPU API를 사용하여 구현 가능
            // 현재는 기본 구현만 제공
