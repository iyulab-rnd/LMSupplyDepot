...
public static class LMModelArtifactAnalyzer
{
    private static readonly Regex SplitFilePattern = new(
        @"^(?<base>.+?)-(?<number>\d{5})-of-(?<total>\d{5})\.(?<format>[^.]+)$",
